...
%     ResponseforPLJudgment(keysetup.forward);
    ...
    %正式实验
    ShowInstruction(w,inssetup.start,inssetup.base,pos, mod(subID,2)==1);
    [answer_code,rtime,rtime_FTV,response_code,tasktype] = RunExperiment(w,wRect,length(ftvparas.condition),frame_duration,...
    NumTotalMoivePool,NumSelectedMovie,MovieCntre,act,ftvparas,inssetup,pos,...
    ...
    fprintf(fid,'%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n',...
        ...
        'RT','distance','FTV','RT_FTV','Tasktype');
    for j = 1:length(response_code)
        fprintf(fid,'%s\t%3.0f\t%3.0f\t%3.0f\t%3.0f\t%3.0f\t%3.0f\t%4.4f\t%3.0f\t%3.0f\t%4.4f\t%3.0f\n',...
        name,subID,sex,age,j,str2num(ftvparas.condition{j}(2)),response_code(j)==str2num(ftvparas.condition{j}(2))+1,...
        rtime(j),ftvparas.TrialType(str2num(ftvparas.condition{j}(1))),answer_code(j),rtime_FTV(j),tasktype(j));
    end
    fclose(fid);
    clear all
catch
